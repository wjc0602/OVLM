...
def combine_tokens(template_tokens, search_tokens, text_src=None, mode='direct', return_res=False):
    # [B, HW, C]
    ...
    if mode == 'direct':
        if text_src is not None:
            merged_feature = torch.cat((template_tokens, search_tokens, text_src), dim=1)
        else:
