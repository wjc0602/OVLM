...
class OSTrack(BaseTracker):
    ...
    def track(self, image, info: dict = None):
        ...
        if self.frame_id <= self.hp_template_num:
            ...
            templates = self.select_representatives()
        ...
        # if np.max(pred_score_map.cpu().numpy()) > self.hp_confidence_threshold:
        ...
        if False:
            if not self.use_visdom:
                ...
                self.visdom.register(torch.from_numpy(self.all_memory_frames).permute(2, 0, 1), 'image', 1, 'template')
    ...
    def select_representatives(self):
        memory_len = len(self.all_memory_frames)
        ...
        representatives = []
        for idx in indexes:
            fm = self.all_memory_frames[idx - 1]
            if not fm.is_cuda:
