...
class OSTrackActor(BaseActor):
    ...
    def compute_losses(self, pred_dict, gt_dict, return_status=True):
        ...
        if 'pred_score' in pred_dict:
            score_loss = self.objective['score'](pred_dict['pred_score'], gt_dict['label'].squeeze())
        ...
        if return_status:
            ...
            if 'pred_score' in pred_dict:
                # print(np.round(pred_dict['pred_score'].detach().cpu().numpy(), 2))
                # print(gt_dict['label'].squeeze().detach().cpu().numpy())
