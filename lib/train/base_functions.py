...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "TNL2k", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val",
        ...
        if name == "TNL2k":
            if settings.use_lmdb:
                print("Building lasot dataset from lmdb")
                datasets.append(Lasot_lmdb(settings.env.tnl2k_lmdb_dir, split='train', image_loader=image_loader))
            else:
...
def build_dataloaders(cfg, settings):
    ...
    train_cls = getattr(cfg.MODEL.RESULT_EVAL, "ENABLE", False)
...
def get_optimizer_scheduler(net, cfg):
    train_score = getattr(cfg.MODEL.RESULT_EVAL, "ENABLE", False)
    if train_score:
        print("Only training classification head. Learnable parameters are shown below.")
        param_dicts = [{"params": [p for n, p in net.named_parameters() if "score_head" in n and p.requires_grad]}]
