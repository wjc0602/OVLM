...
# network related
from lib.models.ostrack import build_ostrack, build_ostrack_re
...
def run(settings):
    ...
    # Create network
    if not cfg.MODEL.RESULT_EVAL.ENABLE:
        net = build_ostrack(cfg)
    elif cfg.MODEL.RESULT_EVAL.ENABLE:
        net = build_ostrack_re(cfg)
    else:
    ...
    elif settings.script_name == "ostrack_re":
        focal_loss = FocalLoss()
        objective = {'giou': giou_loss, 'l1': l1_loss, 'focal': focal_loss, 'score': BCEWithLogitsLoss()}
        loss_weight = {'giou': cfg.TRAIN.GIOU_WEIGHT, 'l1': cfg.TRAIN.L1_WEIGHT, 'focal': 1., 'score': 1.0}
        actor = OSTrackActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
