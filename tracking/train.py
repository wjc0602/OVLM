...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='ostrack_re', help='training script name')
    parser.add_argument('--config', type=str, default='momn_3_ts_re_la_384', help='yaml configure file name')
    parser.add_argument('--save_dir', type=str, default='./workspace', help='root directory to save checkpoints, logs, and tensorboard')
    ...
    parser.add_argument('--nproc_per_node', type=int, default='6', help="number of GPUs per node")  # specify when mode is multiple
...
def main():
    ...
    if args.mode == "single":
        ...
        train_cmd = "python -m torch.distributed.launch --nproc_per_node %d --master_addr %s --master_port %d --nnodes %d --node_rank %d lib/train/run_training.py " \
                    "--script %s --config %s --save_dir %s --use_lmdb %d --script_prv %s --config_prv %s --use_wandb %d " \
                    "--distill %d --script_teacher %s --config_teacher %s" \
                    % (args.nproc_per_node, args.ip, args.port, args.world_size, args.rank, args.script, args.config, args.save_dir, args.use_lmdb, args.script_prv,
                       args.config_prv, args.use_wandb,
                       args.distill, args.script_teacher, args.config_teacher)
    else:
